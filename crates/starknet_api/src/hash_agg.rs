...
struct HashAggStats {
    ...
    pedersen_hits: u64,
    pedersen_misses: u64,
    pedersen_pair_inputs: HashSet<(Felt, Felt)>,
    pedersen_array_inputs: HashSet<Vec<Felt>>,
    poseidon_calls: u64,
...
pub fn snapshot() -> HashAggSnapshot {
    ...
    STATS.with(|stats| {
        ...
        HashAggSnapshot {
            ...
            pedersen_inputs: stats.pedersen_pair_inputs.len() as u64
                + stats.pedersen_array_inputs.len() as u64,
...
#[inline]
pub fn record_pedersen_pair_call(left: Felt, right: Felt, hit: bool) {
    if !enabled() {
        return;
    }
    STATS.with(|stats| {
        ...
        stats.pedersen_pair_inputs.insert((left, right));
    });
}

#[inline]
pub fn record_pedersen_array_call(values: &[Felt], hit: bool) {
    if !enabled() {
        return;
    }
    STATS.with(|stats| {
        ...
        stats.pedersen_array_inputs.insert(values.to_vec());
    });
}
