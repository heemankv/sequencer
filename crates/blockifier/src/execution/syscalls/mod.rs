...
use self::hint_processor::{
    BLOCK_NUMBER_OUT_OF_RANGE_ERROR, EmitEventError, SyscallExecutionError, SyscallHintProcessor,
    create_retdata_segment, execute_inner_call, execute_library_call, felt_to_bool,
    read_call_params, read_calldata, read_felt_array, write_segment,
};
...
impl SyscallRequest for SendMessageToL1Request {
    ...
    fn read(vm: &VirtualMachine, ptr: &mut Relocatable) -> SyscallResult<SendMessageToL1Request> {
        let to_address = ContractAddress::try_from(felt_from_ptr(vm, ptr)?)?;
